...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t reboot = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            ESP_LOGI(TAG, "Config done, wifi_board will handle switchover");
            return ESP_OK;
        },
        .user_ctx = this
    };
    ...
    // Register the /exit endpoint (for mini program / mobile app WiFi config)
    httpd_uri_t exit_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            auto* this_ = static_cast<WifiConfigurationAp*>(req->user_ctx);
            this_->exit_requested_ = true;
            ESP_LOGI(TAG, "Exit config mode requested, wifi_board will handle switchover");
            return ESP_OK;
        },
        .user_ctx = this
    };
    ...
    // ================================================================
    // CAPTIVE_PORTAL_MODE: 1=popup(redirect), 0=silent(fake internet OK)
    // ================================================================
#define CAPTIVE_PORTAL_MODE 1

#if CAPTIVE_PORTAL_MODE
    auto captive_portal = [](httpd_req_t *req) -> esp_err_t {
        auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &apple_handler));
    auto android_204 = [](httpd_req_t *req) -> esp_err_t {
    ...
    httpd_uri_t android_handler = {
        .uri = "/generate_204*", .method = HTTP_GET,
        .handler = android_204, .user_ctx = NULL
    };
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &android_handler));
    const char* silent_urls[] = {
        "/mobile/status.php", "/check_network_status.txt",
        "/ncsi.txt", "/fwlink/", "/connectivity-check.html",
        "/success.txt", "/portal.html", "/library/test/success.html"
    };
    for (const auto& url : silent_urls) {
        httpd_uri_t uri = { .uri = url, .method = HTTP_GET, .handler = android_204, .user_ctx = NULL };
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &uri));
    }
#endif
